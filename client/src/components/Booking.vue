...
onMounted( async () => {
// Venir vérifier si la séance existe bien sinon redirection
// récupérer toutes les places de la séances. 
    ...
    if (session.value.id == null || new Date(session.value.session_datetime) < new Date() ){
        location.href = "/";
    }

    await fetchBookings();
    bookings.value = bookings.value.filter( (e,i) => e.session_id == `/movie_screenings/${id}` )

})

const fetchSession = async (id) => {
    return fetch(`https://localhost/movie_screenings/${id}`)
    .then((response) => response.json())
    .then((data) => (session.value = data));
}   

const fetchBookings = async () => {
    return fetch(`https://localhost/bookings/`)
    .then((response) => response.json())
